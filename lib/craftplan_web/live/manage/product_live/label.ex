  ...
  def render(assigns) do
    ...
    <div class="mx-auto max-w-3xl bg-white p-6 print:m-0 print:max-w-full print:border-0 print:p-0 print:shadow-none">
      <div class="mb-4 flex items-start justify-between print:mb-2">
        <div>
          <h1 class="text-2xl font-semibold print:text-xl">{gettext("Product Label")}</h1>
          <div class="text-sm text-stone-600">{gettext("SKU")}: {@product.sku}</div>
        </div>
        <div class="text-right text-sm">
          <div class="text-stone-600">{gettext("Date")}</div>
          <div class="font-medium">{format_date(@today, format: "%Y-%m-%d")}</div>
          <div class="mt-2 text-stone-600">{gettext("Batch")}</div>
      ...
      <div :if={@ingredients != []} class="mb-4">
        <div class="mb-1 text-sm font-medium text-stone-700">{gettext("Ingredients")}</div>
      ...
      <div :if={@allergens != []} class="mb-4">
        <div class="mb-1 text-sm font-medium text-stone-700">{gettext("Allergens")}</div>
      ...
      <div class="mt-6 flex justify-end print:hidden">
        <.button variant={:primary} onclick="window.print()">{gettext("Print")}</.button>
      </div>
    </div>
    """
  end

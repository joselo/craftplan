defmodule CraftplanWeb.SettingsLive.ApiKeysComponent do
  ...
  def render(assigns) do
    ...
    <div class="space-y-6">
      <.header>
        ...
        <:actions>
          <.button type="button" variant={:primary} phx-click="show_create_modal" phx-target={@myself}>
            <.icon name="hero-plus" class="mr-2 -ml-1 h-4 w-4" /> {gettext("Create API Key")}
          </.button>
        </:actions>
      </.header>
      ...
      <div class="rounded-md border border-gray-200 bg-white">
        <div class="p-4">
          <.table id="api-keys" rows={@api_keys} wrapper_class="mt-0">
            ...
            <:col :let={key} label={gettext("Prefix")}>
              <code class="text-xs">{key.prefix}...</code>
            </:col>
            <:col :let={key} label={gettext("Scopes")}>
            ...
            <:empty>
              <div class="py-6 text-center text-sm text-stone-500">
                {gettext("No API keys yet. Create one using the button above.")}
              </div>
            </:empty>
          </.table>
        </div>
      ...
      <.modal
        ...
        description={gettext("Name your key and select which resources it can access")}
        ...
        >
          ...
          <div class="mt-4">
            <label class="text-sm font-medium text-stone-700">
              {gettext("Resource permissions")}
            </label>
            <p class="mb-3 text-xs text-stone-500">
              {gettext("Select read and/or write access for each resource.")}
            </p>
    ...
    case AshPhoenix.Form.submit(socket.assigns.form, params: params) do
      {:ok, api_key} ->
        ...
        {:noreply,
         ...
         |> assign(:scope_selections, %{})
         |> put_flash(:info, gettext("API key created"))}
  ...
  defp format_scopes_summary(scopes) when is_map(scopes) do
    ...
    case count do
      0 -> gettext("No access")
      n -> ngettext("%{count} resource", "%{count} resources", n)
    end
  end

  defp format_scopes_summary(_), do: gettext("No access")
